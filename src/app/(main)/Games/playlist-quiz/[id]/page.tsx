...
const normalizeString = (str: string) => {
  ...
  return simplified
    ...
    .replace(/-.*$/g, "") // Remove content after hyphen
...
const PlaylistQuizGame = () => {
  const params = useParams();
  const searchParams = useSearchParams();
  const router = useRouter();
  const playlistId = params.id as string;
  const playlistName = searchParams.get("name") || "Playlist Quiz";
  ...
  useEffect(() => {
    const initGame = async () => {
      ...
      setLoading(true);
      const playlist = await fetchPlaylistDetails(playlistId, token);

      if (playlist && playlist.tracks.items) {
        // Extract real tracks and filter out any null tracks
        const tracks = playlist.tracks.items
          .map((item) => item.track)
          .filter((track) => track !== null);

        // Shuffle and take top 10 or all if less
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-zinc-400">Loading playlist tracks...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center space-y-6">
        ...
        <p className="text-zinc-400 max-w-md">
          This playlist seems to be empty or private.
        </p>
        <Button
          onClick={() => router.back()}
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardContent className="p-8 space-y-6">
              ...
              <p className="text-zinc-400">
                Accuracy on {gameTracks.length} songs from "{playlistName}"
              </p>
          ...
          <CardContent className="p-8 md:p-12 flex flex-col items-center space-y-10">
            <div className="relative w-64 h-64 flex items-center justify-center">
              ...
              >
                ...
                >
                  {!showAnswer && isCurrentSongPlaying && (
...
export default PlaylistQuizGame;
